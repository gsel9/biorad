...
def _calc_score(selector, X, y, indices, **fit_params):
    ...
    _, ncols = np.shape(X[:, indices])
    if 'n_components' in selector.est_.get_params():
        if selector.est_.n_components > ncols:
            selector.est_.n_components = int(ncols - 1)
        if selector.est_.n_components < 1:
            selector.est_.n_components = 1
...
class SequentialFeatureSelector(BaseEstimator, MetaEstimatorMixin):
    ...
    def _check_params(self, X):
        ...
        _, ncols = np.shape(X)
        if 'n_components' in self.estimator.get_params():
            if self.estimator.n_components > ncols:
                self.estimator.n_components = int(ncols - 1)
            elif self.estimator.n_components < 1:
